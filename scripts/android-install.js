...
module.exports = function (context) {
    ...
    if (plugins.length > 0 && plugins.indexOf('cordova-plugin-dingding') === -1) {
        return;
    }
    ...
    var targetDir = path.join(projectRoot, "platforms", "android", "src", packageName.replace(/\./g, path.sep), "ddshare");

    if (!fs.existsSync(targetDir)) {
        targetDir = path.join(projectRoot, "platforms", "android", "app", "src", "main", "java", packageName.replace(/\./g, path.sep), "ddshare");
    }
